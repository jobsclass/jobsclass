-- ============================================
-- 주문 및 결제 시스템 스키마 (FINAL - 실제 테이블 구조 기반)
-- ============================================
...
-- 인덱스 생성
CREATE INDEX IF NOT EXISTS idx_customers_service_id ON customers(service_id);
CREATE INDEX IF NOT EXISTS idx_customers_status ON customers(status);

-- ============================================

-- 2. 주문 테이블 (orders가 이미 있으면 DROP 후 재생성)
...
-- 4. 구독 정보 테이블 (subscriptions가 이미 있으면 DROP 후 재생성)
...
-- 5. AI 사용 내역 테이블 (ai_usage_logs가 이미 있으면 DROP 후 재생성)
...
DO $$ 
BEGIN
  RAISE NOTICE '✅ 마이그레이션 완료! (FINAL)';
  RAISE NOTICE '';
  RAISE NOTICE '업데이트된 테이블:';
  RAISE NOTICE '  - customers (service_id, status, company, message 추가)';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '';
  RAISE NOTICE '다음: Vercel 환경 변수 설정 → 배포';
END $$;
